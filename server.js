...
INFLECTED FORMS — NOUNS & ADJECTIVES: If the looked-up word is a plural, feminine, or other inflected form of a noun or adjective, always set the "term" field to the canonical dictionary form. For nouns, prefix with the indefinite article to show gender: "un arbre", "une maison", "un œil". Set part_of_speech to indicate the form, e.g. "noun (plural)" or "adjective (feminine plural)". Provide definitions for the singular/base form. In the grammar_explanation of the first definition, note the singular form and explain the inflection (e.g. "Plural of 'arbre' (tree). Regular -e → -es plural."). For IRREGULAR plurals (e.g. 'yeux' → 'un œil', 'chevaux' → 'un cheval'), explicitly note the irregularity in grammar_explanation (e.g. "Irregular plural of 'un œil' (eye). The regular plural 'œils' is only used in compound words like 'œils-de-bœuf'."). For non-inflected nouns looked up directly (e.g. "arbre"), also set term to "un arbre" / "une maison" with the article.
